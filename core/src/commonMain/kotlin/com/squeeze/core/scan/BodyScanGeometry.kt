...
class ScaleRecovery(
    ...
    /**
     * Calibration factor applied to the width-to-centimetre conversion.
     *
     * The selfie segmenter consistently over-counts body width by a small amount due to
     * clothing padding, segmentation edge effects and mask defects. This systematic bias
     * scales all circumferences proportionally and can be calibrated once against tape
     * measurements. A value below 1.0 reduces measurements; 1.0 applies no correction.
     *
     * The factor is applied to *all* width-to-centimetre conversions, including the assumed
     * depth for front-only scans, so it corrects the scale without altering anatomical site
     * detection. The value 0.985 compensates for the observed ~1.5% over-estimation in the
     * silhouette extraction pipeline.
     */
    private val biasCorrection: Double = 1.0,
) {
    init {
        ...
        require(biasCorrection in 0.85..1.15) {
            "bias correction outside safe range: $biasCorrection"
        }
    }
    ...
    fun widthToCm(widthFraction: Double, imageAspectRatio: Double): Double {
        ...
        return widthFraction * imageAspectRatio * cmPerHeightFraction * biasCorrection
    }
